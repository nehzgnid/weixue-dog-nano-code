...
def _resolve_cfg_path(config_arg: str, repo_root: Path) -> Path:
    raw = Path(config_arg)
    if raw.is_absolute():
        return raw
    candidates = [
...
def _resolve_artifact_path(path_str: str, cfg_path: Path, repo_root: Path) -> Path:
    raw = Path(path_str)
    if raw.is_absolute():
        return raw
    candidates = [
...
def main():
    ...
    parser.add_argument("--config", type=str, default="rl/config/wavego_deploy_config.yaml")
    ...
    cfg_path = _resolve_cfg_path(args.config, repo_root)
    ...
    normalizer_path = _resolve_artifact_path(cfg["model"]["normalizer_path"], cfg_path, repo_root)
    ...
    onnx_path = _resolve_artifact_path(cfg["model"]["onnx_path"], cfg_path, repo_root)
    policy = ONNXPolicy(
        str(onnx_path),
