...
if __name__ == "__main__":
    if not TK_AVAILABLE:
        print("[Error] tkinter 未安装，无法启动 simulation_step_in_place GUI。请安装 python3-tk 后重试。")
        raise SystemExit(1)
    if not MATPLOTLIB_AVAILABLE:
        print("[Error] matplotlib 未安装，无法启动 simulation_step_in_place GUI。请安装 python3-matplotlib 后重试。")
        raise SystemExit(1)
    app = StepSimApp()
    app.mainloop()